...
public class CalorimeterPlots {
	...
	public CalorimeterPlots(WithoutCanvas canvas,  String name){
		this.canvas = null;
	...
	public void createHistogramsWithoutCanvas(WithoutCanvas canvas) {
		canvas.create2DHisto( detectorName+"SF", detectorName+" SF", "p", "E/p", 100, 0, 10, 100, 0, 0.4);
		canvas.create2DHisto( detectorName+"SF-elec", detectorName+" SF elec", "p", "E/p", 100, 0, 10, 100, 0, 0.4);
		canvas.create2DHisto( detectorName+"SF-phot", detectorName+" SF phot", "p", "E/p", 100, 0, 10, 100, 0, 0.4);
		canvas.create2DHisto( detectorName+"SF-prot", detectorName+" SF prot", "p", "E/p", 100, 0, 10, 100, 0, 0.4);
		canvas.create2DHisto( detectorName+"EvsT", detectorName+" E vs T", "E", "T", 50, 0, 4, 50, 0, 1000);
		canvas.create2DHisto( detectorName+"EvsT-elec", detectorName+" E vs T elec", "E", "T", 50, 0, 4, 50, 0, 1000);
		canvas.create2DHisto( detectorName+"EvsT-phot", detectorName+" E vs T phot", "E", "T", 50, 0, 4, 50, 0, 1000);
		canvas.create2DHisto( detectorName+"EvsT-prot", detectorName+" E vs T prot", "E", "T", 50, 0, 4, 50, 0, 1000);
		
		canvas.create2DHisto( detectorName+"TelecvsTphot", detectorName+"Telec vs Tphot", "Tphot", "Telec", 100, 150, 300, 100, 150, 300);
		canvas.create2DHisto( detectorName+"TelecvsTphot"+1, detectorName+"Telec vs Tphot"+" s"+1, "Tphot", "Telec", 100, 150, 300, 100, 150, 300);
		canvas.create2DHisto( detectorName+"TelecvsTphot"+4, detectorName+"Telec vs Tphot"+" s"+4, "Tphot", "Telec", 100, 150, 300, 100, 150, 300);
		canvas.create2DHisto( detectorName+"TelecvsTphot"+7, detectorName+"Telec vs Tphot"+" s"+7, "Tphot", "Telec", 100, 150, 300, 100, 150, 300);
		
		canvas.create2DHisto( detectorName+"TelecvsTprot", detectorName+"Telec vs Tprot", "Tprot", "Telec", 100, 150, 300, 100, 150, 300);
		
		canvas.create2DHisto( detectorName+"TprotvsTphot", detectorName+"Tprot vs Tphot", "Tphot", "Tprot", 100, 150, 300, 100, 150, 300);
	}
	...
	public void fillHistogramsWithoutCanvas(Event event){
		CalorimeterEvent caloEvent = event.getForwardEvent().getCalorimeterEvent();
		for (CalorimeterRecCluster caloClusters : caloEvent.getCalorimeterClusters()){
			this.myCanvas.fill2DHisto(detectorName+"EvsT", caloClusters.getEnergy(), caloClusters.getTime());
		}
		ParticleEvent particleEvent = event.getParticleEvent();
		for (Particle particle : particleEvent.getParticles()){
			if (particle.hasCalorimeterClusters()>0){
				...
				this.myCanvas.fill2DHisto(detectorName+"SF", particle.getMomentum().mag(), energy/particle.getMomentum().mag());	
			}
		}
		for (Electron particle : particleEvent.getElectrons()){
			if (particle.hasCalorimeterClusters()>0){
				...
				this.myCanvas.fill2DHisto(detectorName+"SF-elec", particle.getMomentum().mag(), energy/particle.getMomentum().mag());
				this.myCanvas.fill2DHisto(detectorName+"EvsT-elec", particle.getCalorimeterRecClusters().get(0).getEnergy(), particle.getCalorimeterRecClusters().get(0).getTime());
			}
			for (Photon particle2 : particleEvent.getPhotons()){
				if (particle.hasCalorimeterClusters()>0 && particle2.hasCalorimeterClusters()>0){
					this.myCanvas.fill2DHisto(detectorName+"TelecvsTphot", particle2.getCalorimeterRecClusters().get(0).getTime(), particle.getCalorimeterRecClusters().get(0).getTime());
					for (int i=0; i<particle.hasCalorimeterClusters(); i++){
						for (int j=0; j<particle.hasCalorimeterClusters(); j++){
							if (particle.getCalorimeterRecClusters().get(i).getLayer()==particle.getCalorimeterRecClusters().get(j).getLayer()){
								this.myCanvas.fill2DHisto(detectorName+"TelecvsTphot"+particle.getCalorimeterRecClusters().get(i).getLayer(), particle2.getCalorimeterRecClusters().get(0).getTime(), particle.getCalorimeterRecClusters().get(0).getTime());
							}
						}
					}
				}
			...
			for (Proton particle2 : particleEvent.getProtons()){
				if (particle.hasCalorimeterClusters()>0 && particle2.hasCalorimeterClusters()>0){
					this.myCanvas.fill2DHisto(detectorName+"TelecvsTprot", particle2.getCalorimeterRecClusters().get(0).getTime(), particle.getCalorimeterRecClusters().get(0).getTime());
				}
			}
		}
		for (Photon particle : particleEvent.getPhotons()){
			if (particle.hasCalorimeterClusters()>0){
				...
				this.myCanvas.fill2DHisto(detectorName+"SF-phot", particle.getMomentum().mag(), energy/particle.getMomentum().mag());
				this.myCanvas.fill2DHisto(detectorName+"EvsT-phot", particle.getCalorimeterRecClusters().get(0).getEnergy(), particle.getCalorimeterRecClusters().get(0).getTime());
			}
			for (Proton particle2 : particleEvent.getProtons()){
				if (particle.hasCalorimeterClusters()>0 && particle2.hasCalorimeterClusters()>0){
					this.myCanvas.fill2DHisto(detectorName+"TprotvsTphot", particle.getCalorimeterRecClusters().get(0).getTime(), particle2.getCalorimeterRecClusters().get(0).getTime());
				}
			}
		}
		for (Proton particle : particleEvent.getProtons()){
			if (particle.hasCalorimeterClusters()>0){
				...
				this.myCanvas.fill2DHisto(detectorName+"SF-prot", particle.getMomentum().mag(), energy /particle.getMomentum().mag());
				this.myCanvas.fill2DHisto(detectorName+"EvsT-prot", particle.getCalorimeterRecClusters().get(0).getEnergy(), particle.getCalorimeterRecClusters().get(0).getTime());
			}
		}
	}
